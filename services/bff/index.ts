import { BFF_BASE_URL } from '@/constants/env.config'
...
export const bffApiService = async <T = any>(url: string, options?: RequestInit, _retried = false): Promise<T> => {
    ...
    const res = await fetch(`${BFF_BASE_URL}/api/v1${url}`, {
        ...options,
    ...
    if (
        ...
        const refreshRes = await fetch(`${BFF_BASE_URL}/api/v1${REFRESH_PATH}`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                Accept: 'application/json'
